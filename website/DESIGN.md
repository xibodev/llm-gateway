...
- Failure crimson: `#b42318`
- Failure soft field: `#fef3f2`
- Failure detail on dark fields: `#ffb4ab`
...
- Optical Rail Splitter: exact projections from `brand/`, with a matching
  dark-tile favicon and purpose-sized manifest icons. Header links show `llmgw`;
  their accessible names begin with `llmgw` and expand `llm-gateway`.
...
- Routing board: labelled endpoint and provider nodes connected by directional
  rails. Crimson communicates a failed hop, green a served hop, with text labels
  so color is never the only signal.
- Failure crimson and success green are semantic colors, not brand colors.
...
- Social preview: deterministic 1200x630 PNG projected from the canonical SVG;
  the homepage references the absolute PNG URL.
