...
| `ERROR-001C2` | `queued` | After `ERROR-001C1`, harden GCP token exchange and Copilot/Codex device, token, refresh, and revoke error producers. | Sanitize and bound provider-controlled diagnostics at construction while retaining safe status/code; preserve successful device, poll, exchange, refresh, identity, and token-storage behavior; cover each auth package with synthetic tests. |
| `ERROR-001C3` | `queued` | After `ERROR-001C2`, harden generic and legacy OAuth/Copilot browser API projections. | Sanitize every projected error before limiting; preserve semantic poll status and successful device/verification values; never expose access, refresh, or ID tokens; cover user/admin and legacy handlers. |
| `ERROR-001C4` | `queued` | After `ERROR-001C1`, protect provider-check, audit, and outbox free text at persistence and read boundaries. | Sanitize check detail/model, nested audit detail, and outbox error/payload strings before persistence and again on read; preserve status, generation, lease, and retry semantics; test historical unsafe rows. |
| `ERROR-001C5` | `queued` | After `ERROR-001C1`, remove Edge TTS credential-bearing HTTP/websocket URLs from diagnostics and retain handshake status. | Keys, signatures, connection IDs, and full URLs never enter errors/API/verification/telemetry; failed response bodies close; final status survives; skew retry and successful synthesis/catalog behavior remain unchanged. |
| `ERROR-001C6` | `queued` | After `ERROR-001C1`, replace optional raw body-log persistence with bounded sanitized diagnostic snapshots. | Metadata-only default remains unchanged; opt-in snapshots sanitize before limiting, safely handle complete JSON and incomplete/oversized content, preserve request/response/SSE behavior and complete byte counts, and leave no test artifacts. |
