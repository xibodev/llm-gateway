...
| `ERROR-001B` | `queued` | After `ERROR-001A`, normalize non-chat proxy failures. Audio and embeddings currently forward non-2xx upstream bodies directly. | Convert only non-2xx audio/embedding responses to the safe provider-error envelope while preserving original status; successful audio bytes and embedding JSON remain unchanged; cover both failure and success paths. |
| `ERROR-001C` | `queued` | After `ERROR-001A`, protect authentication and secondary diagnostic sinks, including GCP token exchange, Copilot/Codex OAuth errors, Edge TTS handshake URLs, audit/outbox free text, and optional body logs. | Redact before truncation/persistence, retain safe status/code, preserve intentional successful OAuth/device/poll values, and add sink-specific synthetic regression tests. |
...
1. Close the finite current defects: `DEPS-001`, `IAM-002`, `ERROR-001A`,
   `ERROR-001B`, `ERROR-001C`, `PLAY-002`, `DEPLOY-001`, `GOV-001`, `CFG-001`,
   `HTTP-001`, and `HTTP-002`.
