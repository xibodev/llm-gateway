...
| `DEPS-001` | `ready` | Clear known advisories from the console build dependency tree. `npm audit` currently reports fixable findings rooted in Vite and its transitive dependencies. Node is build-time only, but the release build still executes this toolchain. | Update the smallest compatible dependency set; regenerate and verify `package-lock.json` and embedded `dist`; `npm audit`, console checks, Go checks, and the container build pass; review release notes for build/config changes before accepting the update. |
...
| `IAM-002` | `ready` | Preserve every project policy field through console edits. The current editor omits daily/monthly model-credit fields and sends a replacement object that clears them. | Add all backend-supported fields or use an explicit patch contract; round-trip nonzero request, token, cost, credit, and allowlist values through the UI/API without loss; add a regression test proving an unrelated edit cannot zero hidden fields. |
| `ERROR-001` | `ready` | Enforce credential-safe upstream errors across every provider and telemetry path. Native Anthropic, Ollama, and Google paths do not consistently apply the redactor before details reach responses, verification output, or failover telemetry. | Centralize safe error construction; preserve useful status/code without raw credential-shaped content; test every provider family plus verification and telemetry with token/email fixtures; make the `SECURITY.md` claim true. |
...
| `CLAUDE-005` | `queued` | `CLAUDE-001`, `HTTP-002`, `ERROR-001` | Correct Anthropic streaming and error semantics. Resolve a stream before committing HTTP 200; propagate mid-stream failures as Anthropic error events without a false `message_stop`; preserve native thinking/signature/tool/usage events; retain upstream HTTP status in safe Anthropic-shaped errors; replace buffered Responses emulation with real event translation or mark the path non-streaming for `CLAUDE-003` to advertise honestly. This is the Claude-specific completion slice of `STREAM-001`. |
...
1. Close the finite current defects: `DEPS-001`, `IAM-002`, `ERROR-001`,
   `PLAY-002`, `DEPLOY-001`, `GOV-001`, `CFG-001`, `HTTP-001`, and `HTTP-002`.
...
3. Implement `CLAUDE-002` and `CLAUDE-004` in parallel. Follow `CLAUDE-004` with
   `CLAUDE-006`; build `CLAUDE-005` after `HTTP-002` and `ERROR-001`; then close
