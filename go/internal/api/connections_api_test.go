...
func TestAnthropicSetupTokenConnectionIsValidatedAndWriteOnly(t *testing.T) {
	...
	status, rejected := jsonRequest(t, server.URL+"/admin/api/principals/"+human.ID+"/connections", http.MethodPost, "admin-secret", map[string]any{
		"provider_id": "anthropic", "credential_kind": "setup_token", "secret": AnthropicSetupTokenPrefix + "short",
	})
	if status != 400 || !strings.Contains(fmt.Sprint(rejected), "too short") {
		t.Fatalf("expected too short error, got %d: %v", status, rejected)
	}
