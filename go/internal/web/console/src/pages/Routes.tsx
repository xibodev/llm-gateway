...
function choicesFrom(payload: JSONRecord): ModelChoice[] {
  return asList(payload.data).map(asRecord).flatMap((row) => {
    ...
    if (slash <= 0 || slash === id.length - 1 || stringValue(row.owned_by) === "category") return [];
...
export function Routes({ data, mode, detail, onChanged, onNavigate }: { data: JSONRecord; mode: ConsoleMode; detail: string; onChanged: () => Promise<void>; onNavigate: (page: "routes", detail?: string) => void }) {
  ...
  const routes = Object.entries(categories).map(([routeName, value]) => ({
    name: routeName,
  ...
  // Categories cannot nest inside a route, so they never appear as members.
  ...
  const save = async (event: Event) => {
    ...
    setBusy(true);
    try {
      await sendJSON<JSONRecord>("admin", `/categories${principalQuery}`, "POST", { name: name.trim(), failover: members });
      await onChanged();
      setEditing(false);
