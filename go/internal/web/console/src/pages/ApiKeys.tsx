...
import { Check, Copy, KeyRound, Pencil, Plus, Save, ShieldAlert, Trash2, X } from "lucide-preact";
...
function CopyOnce({ token, onDismiss, returnFocus }: { token: string; onDismiss: () => void; returnFocus: HTMLElement | null }) {
  ...
  return <div class="dialog-backdrop" role="presentation"><section ref={dialogRef} class="dialog key-secret-dialog" role="dialog" aria-modal="true" aria-labelledby="key-secret-title" tabIndex={-1}><header><div><p class="eyebrow">Copy once</p><h2 id="key-secret-title">New gateway API key</h2></div><button class="icon-button" type="button" aria-label="Close key secret" onClick={onDismiss}><X size={18} /></button></header><p class="key-secret-warning"><ShieldAlert size={17} /> This value is shown once. It cannot be recovered after this dialog closes.</p><pre class="technical key-secret-value">{token}</pre><footer><button class="button button--secondary" type="button" data-dialog-initial-focus onClick={() => void copy()}>{copied ? <Check size={16} /> : <Copy size={16} />}{copied ? "Copied" : "Copy key"}</button><button class="button button--primary" type="button" onClick={onDismiss}>I saved this key</button></footer></section></div>;
}
...
export function ApiKeys({ data, mode, onChanged }: { data: JSONRecord; mode: ConsoleMode; onChanged: () => Promise<void> }) {
  ...
  const create = async (event: Event) => {
    ...
    try {
      ...
      if (!token) throw new Error("The server did not return the one-time key value.");
      setSecret(token);
      setCreating(false);
      await onChanged();
  ...
  return (
    <div class="page-stack">
      ...
      {keys.length ? <section class="surface table-wrap"><table><thead><tr><th>Name</th><th>Prefix</th><th>Project</th><th>Principal</th><th>Policy</th><th>Status</th><th>Actions</th></tr></thead><tbody>{keys.map((key) => {
        ...
        return <tr key={stringValue(key.id)}><td><KeyRound size={15} /> {stringValue(key.name, "Gateway key")}</td><td class="technical">{stringValue(key.prefix, "Hidden")}</td><td>{stringValue(key.project, stringValue(key.project_id))}</td><td>{stringValue(key.principal, stringValue(key.principal_id))}</td><td>{numberValue(policy.rpm, numberValue(key.rpm)) || numberValue(policy.daily_requests, numberValue(key.daily_requests)) ? `${numberValue(policy.rpm, numberValue(key.rpm)) || "—"} rpm · ${numberValue(policy.daily_requests, numberValue(key.daily_requests)) || "—"} daily` : "Unrestricted"}</td><td><span class={`status-pill ${statusClass}`} title={revoked ? "Permanently revoked" : undefined}>{status}</span></td><td><div class="table-actions"><button class="icon-button" type="button" aria-label={`Edit ${stringValue(key.name)}`} disabled={busy || revoked} onClick={() => edit(key)}><Pencil size={15} /></button>{!revoked ? <button class="button button--secondary" type="button" disabled={busy} onClick={() => void update(key, active)}>{active ? "Disable" : "Enable"}</button> : null}<button class="icon-button" type="button" aria-label={`Revoke ${stringValue(key.name)}`} disabled={busy || revoked} onClick={() => void revoke(key)}><Trash2 size={15} /></button></div></td></tr>;
      })}</tbody></table></section> : null}
      {secret ? <CopyOnce token={secret} onDismiss={() => setSecret("")} returnFocus={createButtonRef.current} /> : null}
    </div>
  );
}
