...
func (rt *Runtime) coreVerticals() map[string]coreVertical {
	...
	for _, extType := range []string{
		ExtensionTypeCodex,
		ExtensionTypeAntigravity,
		ExtensionTypeCopilot,
		ExtensionTypeEdgeTTS,
		ExtensionTypeZenAnonymous,
		ExtensionTypeAnthropicSetup,
	} {
		verticals[extType] = rt.extensionCoreVertical(extType)
	}
	return verticals
}
