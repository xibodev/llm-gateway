...
// revision 0, which has no account state yet. Codex and Antigravity are the
// exception: they report the credential their operation held (see
// credentialCollector), at revision 0 too, and nil only when it never held
// one. The guards refuse revision 0, but callers let any non-nil observation
// replace the one they held, so dropping revision 0 from them would change
