...
func anthropicTestBody(r *http.Request) map[string]any {
...
func TestAnthropicRequestsResolveTheFactoryPrecedence(t *testing.T) {
	...
	base := anthropicServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		mu.Unlock()
		if anthropicTestBody(r)["stream"] == true {
	...
	if _, err := iam.PutSystemProviderConnection(anthropicFixtureInstance, "api_key", "system-key"); err != nil {
		t.Fatal(err)
	}
