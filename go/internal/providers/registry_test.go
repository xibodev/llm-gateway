...
func TestOpenCodeZenAPIKeyIsRequiredAndOnboardable(t *testing.T) {
	entry, ok := RegistryProvider("opencode_zen")
	if !ok || !entry.RequiresAPIKey || !slices.Contains(entry.AuthMethods, "api_key") || !slices.Contains(entry.OnboardingFields, "api_key") {
		t.Fatalf("OpenCode Zen key contract=%+v", entry)
	}
}
